...
var node_modules = "" + ROOT_PATH + "/node_modules";
// 模板目录
...
function createProject(project) {
    ...
    vfs.src(page).pipe(vfs.dest(project + "/page"));

    // 拷贝 framework 目录文件
    var widget = path.join(APP_PATH + "/framework", "**");
    vfs.src(widget).pipe(vfs.dest(project + "/framework"));

    // 拷贝 lib 目录文件
    var widget = path.join(APP_PATH + "/lib", "**");
    vfs.src(widget).pipe(vfs.dest(project + "/lib"));

    // 拷贝配置文件
    var config = [
    //APP_PATH + '/stbui-conf.js',
    APP_PATH + "/README.md", APP_PATH + "/package.json"];
    vfs.src(config).pipe(vfs.dest(project));
...
function find_pc(project) {
    var templatePath = "" + node_modules + "/stbui-pc";
    var pagePath = "" + templatePath + "/page";
    var frameworkPath = "" + templatePath + "/framework";
    var libPath = "" + templatePath + "/lib";
    ...
    "" + templatePath + "/README.md", "" + templatePath + "/package.json"];
    vfs.src(config).pipe(vfs.dest(project));
...
function find_h5() {
    var templatePath = "" + node_modules + "/stbui-h5";
